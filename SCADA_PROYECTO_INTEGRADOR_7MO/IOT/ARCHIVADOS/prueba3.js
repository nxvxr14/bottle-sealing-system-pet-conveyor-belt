...
const { Board, Button, Proximity, Pin } = require("johnny-five");
...
board.on("ready", () => {
  ...
  proximity.on("change", () => {
    ...
    volTank = (23.1 - centimeters)*(3.1416)*((radioTank/2)*(radioTank/2));
    });
...
function fillBottle() {
  ...
  if (Date.now() - TIME2 >= 2000 && flagStopCon) {
    //console.log("INICIO");
    board.analogWrite(enableWater, 60);
  }
  ...
  if (localML > volFill) {
    //console.log("FIN");
    board.analogWrite(enableWater, 0);
    if (startTime3) {
      TIME3 = Date.now();
      startTime3 = false;
    }
