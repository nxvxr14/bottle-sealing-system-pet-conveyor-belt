...
// ARDUINO
const arduino1 = "/dev/tty.usbmodem14101";
const arduino2 = "/dev/tty.usbmodem14201"; 
const board = new Board({ port: arduino1 });
const board2 = new Board({ port: arduino2 });

// CONECTION MASTER-SLAVE
let conectArduino2ON = false;
let conectArduino2OFF = false;
...
function coiledBottle(){
  ...
  let elapsedTime = Date.now() - TIME4; // Calcula el tiempo transcurrido desde TIME4

  if ((elapsedTime) >= 5000 && (elapsedTime) <= 5500 && flagStopConvEn ) conectArduino2ON = true;     console.log("SERVO");
  if ((elapsedTime) > 8000 && (elapsedTime) <= 8500 && flagStopConvEn ) board.analogWrite(turnMotor, 240);     console.log("ON");
  if ((elapsedTime) > 13000 && (elapsedTime) <= 13500 && flagStopConvEn ) board.analogWrite(turnMotor, 0);     console.log("OFF");
  if ((elapsedTime) > 14000 && (elapsedTime) <= 14500 && flagStopConvEn ) conectArduino2ON = false;     console.log("OFF");


  

  if (elapsedTime >= 18000 && flagStopConvEn) {
    console.log("FIN");
    board.analogWrite(turnMotor, 0);
    flagStopCon = false;
    flagStopConvEn = false;
    conectArduino2ON = false;
